...
export const getCredentials = (): AuthenticationResponse => {
  const strCredentials = typeof window !== 'undefined' ? window.localStorage.getItem('ACCESS_TOKEN') : null
...
export const saveCredentials = (response: AuthenticationResponse): void => {
  ...
  typeof window !== 'undefined' && window.localStorage.setItem('ACCESS_TOKEN', JSON.stringify(newAccessToken))
}
...
export const isLoggedInState = () => {
  ...
  return !!access_token && !!decodeAccessToken?.user_id && !!decodeAccessToken?.exp && now < decodeAccessToken?.exp
}
...
export const decodeAccessToken = (access_token: string): PartialCredentials => parseJwt(access_token)
