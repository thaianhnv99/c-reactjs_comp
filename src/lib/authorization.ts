...
export const setAuthorization = (token: string) => localStorage?.setItem(ACCESS_TOKEN_KEY, JSON.stringify(token));
...
export const setUserProfile = (user: User | null) => localStorage?.setItem(USER_PROFILE_KEY, JSON.stringify(user));
