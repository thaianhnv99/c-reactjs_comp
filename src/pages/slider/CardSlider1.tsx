...
const CardSlider1 = () => {
  ...
  const loadShow = useCallback(() => {
    ...
    for (var i = activeCount + 1; i < listChild.length; i++) {
      stt++;
    ...
    for (let i = activeCount - 1; i >= 0; i--) {
      ...
      (listChild[i] as HTMLElement).style.opacity = (
        stt > 2 ? 0 : 0.6
      ).toString();
    }
  ...
  useEffect(() => {
    if (listChild?.length > 0) loadShow();
  }, [loadShow, listChild]);
    ...
    >
          ...
          return (
            <Box
              ...
              sx={{
                ...
                left: "calc(50% - 110px)",
                transform:
                  "translateX(120px) scale(0.8) perspective(16px) rotateY(-1deg)",
                transition: "all 1s",
                top: 0,
      ...
      <Box>
        ...
        <Button
          onClick={() => {
            setActiveCount((prev) =>
              prev + 1 < listChild.length ? prev + 1 : prev
            );
            loadShow();
          }}
        >
